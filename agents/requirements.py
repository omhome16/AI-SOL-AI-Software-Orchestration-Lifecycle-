...
CRITICAL: Respond with ONLY valid JSON matching the RequirementsAnalysis schema."""
    ...
    async def _conduct_domain_research(self, requirements: str, classification, user_context: Dict[str, Any]) -> List[
        Dict[str, Any]]:
        ...
        try:
            ...
            self.log(f"Domain research failed: {e}", "warning")

        return research_results

    async def _generate_domain_specific_requirements(self, requirements: str, classification,
                                                     research_results: List[Dict[str, Any]],
                                                     context: AgentContext) -> RequirementsAnalysis:
        """Generate domain-specific requirements using LLM"""

        # Get domain template
        domain_template = self.domain_templates.get(classification.domain, {})

        # Build comprehensive prompt
        prompt = f"""Analyze these user requirements and create comprehensive, domain-specific technical specifications:

**Project Classification:**
- Project Type: {classification.project_type.value}
- Domain: {classification.domain}
- Complexity: {classification.complexity.value}
- Estimated Duration: {classification.estimated_duration_hours} hours
- Team Size: {classification.team_size_recommendation}

**User Requirements:**
{requirements}

**Domain-Specific Context:**
{self._format_domain_context(classification, domain_template)}

**Research Findings:**
{self._format_research_results(research_results)}

**Instructions:**
1. Generate comprehensive functional requirements specific to {classification.domain} domain
2. Include domain-appropriate non-functional requirements
3. Consider industry standards and compliance requirements for {classification.domain}
4. Recommend technology stack suitable for {classification.domain} projects
5. Assess realistic complexity and timeline based on domain expertise
6. Identify domain-specific risks and assumptions
7. Incorporate research findings into recommendations

**CRITICAL: Respond with ONLY valid JSON matching the RequirementsAnalysis schema.**

Generate domain-specific requirements analysis:"""

        try:
            analysis = await self.call_llm_json(prompt, output_schema=RequirementsAnalysis)
            return analysis
        except Exception as e:
            self.log(f"LLM analysis failed: {e}", "error")
            return self._create_fallback_analysis(requirements, classification, domain_template)

    def _format_domain_context(self, classification, domain_template: Dict[str, Any]) -> str:
    ...
    def _create_fallback_analysis(self, requirements: str, classification,
                                  domain_template: Dict[str, Any]) -> RequirementsAnalysis:
